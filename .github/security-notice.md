...
As a rule-of-thumb within the workflows of this repository, always assign any input that _might_ come from a user in either a Github workflow or a composite action into environment variables when using a bash or javascript script as a step in the workflow or action. An example of this can be seen below:
...
Note that, when you are writing Github workflows or composite actions, ensure you follow this code-style to reduce the attack surface that could result from attempted script injection into the workflows. Third-party actions are pinned to a full commit SHA for the same reason.
